...
export type Marcher = {
  id: number; name: string | null; section: string; drillPrefix: string; drillOrder: number;
};
export type Position = {
  marcherId: number; pageId: number; x: number; y: number; rotation: number;
  fillColor?: string | null; visible: boolean; labelVisible: boolean;
};
export type Page = { id: number; index: number; displayNumber: number; startBeat: number; startBeatIndex: number; };
...
export type Field = {
  ...
  xCheckpoints?: Array<{ stepsFromCenterFront: number; name: string; terseName: string; }> ;
  yCheckpoints?: Array<{ stepsFromCenterFront: number; name: string; terseName: string; }> ;
};
export type SectionAppearance = { section: string; fillColor: string | null; shapeType: string | null; };
...
export type Drill = {
  ...
  marchers: Marcher[]; pages: Page[]; beats: Beat[]; positions: Position[];
  field: Field; appearances: SectionAppearance[]; sourceName: string;
};
...
function rows(db: any, sql: string): any[] {
  ...
  if (!result) return [];
  return result.values.map((r: any[]) => Object.fromEntries(result.columns.map((c: string, i: number) => [c, r[i]])));
}

export async function parseDots(buffer: ArrayBuffer, sourceName: string, includeAudioData = true): Promise<Drill> {
  const SQL = await sqlJsPromise;
  const db = new SQL.Database(new Uint8Array(buffer));

  const marchers = rows(db, `SELECT id,name,section,drill_prefix AS drillPrefix,drill_order AS drillOrder FROM marchers ORDER BY id`)
    .map((r: any) => ({...r, id:+r.id, drillOrder:+r.drillOrder}));
  const beats = rows(db, `SELECT id,duration,position FROM beats ORDER BY position`)
    .map((r:any)=>({id:+r.id,duration:+r.duration,position:+r.position}));
  ...
  const pageColumns = rows(db, `PRAGMA table_info(pages)`).map((r:any) => String(r.name));
  let pageRows: any[] = [];
  ...
  const pageNumberColumn = ["page_number", "pageNumber", "number", "display_number", "displayNumber", "page_index", "pageIndex"]
    .find((name) => pageColumns.includes(name));
  const quotedPageNumberColumn = pageNumberColumn ? `, "${pageNumberColumn.replace(/"/g, '""')}" AS displayNumber` : "";
  if (pageColumns.includes("next_page_id") || pageColumns.includes("previous_page_id")) {
    pageRows = rows(db, `SELECT id,start_beat AS startBeat${quotedPageNumberColumn}, next_page_id AS nextPageId, previous_page_id AS previousPageId FROM pages`);
    ...
    else pageRows = [...pageRows].sort((a:any,b:any) => {
    ...
    pageRows = rows(db, `SELECT id,start_beat AS startBeat${quotedPageNumberColumn} FROM pages`)
      .sort((a:any,b:any) => {
        const ap = beatPosition.get(+a.startBeat) ?? Number.MAX_SAFE_INTEGER;
        const bp = beatPosition.get(+b.startBeat) ?? Number.MAX_SAFE_INTEGER;
        return ap - bp || (+a.id - +b.id);
  ...
  try {
    const workspaceForPages = rows(db, `SELECT json_data AS jsonData FROM workspace_settings ORDER BY id LIMIT 1`)[0];
    if (workspaceForPages?.jsonData) {
      const settings = JSON.parse(workspaceForPages.jsonData);
      for (const key of ["firstPageNumber", "startingPageNumber", "startPageNumber", "pageNumberStart", "pageNumberOffset"]) {
  ...
  const pages = pageRows.map((r:any, i:number)=>({
    ...
    displayNumber: Number.isFinite(Number(r.displayNumber))
      ...
      : (workspacePageStart != null ? workspacePageStart + i : i)
  }));
  const positions = rows(db, `SELECT marcher_id AS marcherId,page_id AS pageId,x,y,rotation_degrees AS rotation,fill_color AS fillColor,visible,label_visible AS labelVisible FROM marcher_pages`)
    .map((r:any)=>({marcherId:+r.marcherId,pageId:+r.pageId,x:+r.x,y:+r.y,rotation:+r.rotation,fillColor:r.fillColor,visible:!!r.visible,labelVisible:!!r.labelVisible}));
  const fp = rows(db, `SELECT json_data AS jsonData FROM field_properties LIMIT 1`)[0];
  const fieldJson = JSON.parse(fp.jsonData);
  const appearances = rows(db, `SELECT section,fill_color AS fillColor,shape_type AS shapeType FROM section_appearances ORDER BY id`);

  let audioOffsetSeconds = 0;
  let lastPageCounts = 0;
  try {
    const workspace = rows(db, `SELECT json_data AS jsonData FROM workspace_settings ORDER BY id LIMIT 1`)[0];
  ...
  try {
    const utility = rows(db, `SELECT last_page_counts AS lastPageCounts FROM utility ORDER BY id LIMIT 1`)[0];
  ...
  if (includeAudioData) {
    try {
      const audioRow = rows(db, `SELECT id,path,nickname,data,selected FROM audio_files WHERE selected = 1 ORDER BY id LIMIT 1`)[0];
      if (audioRow?.data) {
        audio = {
          id: +audioRow.id,
          path: audioRow.path ?? "",
          ...
          data: audioRow.data instanceof Uint8Array ? audioRow.data : new Uint8Array(audioRow.data),
        };
      }
    } catch {
  ...
  return {
    ...
    audioOffsetSeconds,
    lastPageCounts,
    marchers, pages, beats, positions,
    appearances,
    field: {
